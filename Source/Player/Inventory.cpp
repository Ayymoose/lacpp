...
namespace Zelda
{
    ...
    void Inventory::getDungeonMap(Dungeon dungeon, DungeonMapEntry (&dungeonMapEntry)[DUNGEON_MAX_BLOCKS_X][DUNGEON_MAX_BLOCKS_Y]) const
    {
    ...
    void Inventory::setDungeonMapEntry(const int x, const int y, const DungeonMapEntry& dungeonMapEntry)
    {
    ...
    void Inventory::open()
    {
        //Keyboard::getInstance().resetStates();
        m_open = true;
    }
    ...
    bool Inventory::magicPowder() const
    {
        return m_magicPowder;
    }

    bool Inventory::arrows() const
    {
        return m_arrows;
    }

    bool Inventory::bombs() const
    {
        return m_bombs;
    }

    void Inventory::addItemQuantity(WeaponItem item, int quantity)
    {
        switch (item)
        {
        case WPN_BOMBS:
    ...
    void Inventory::addItem(const InventoryWeapon& inventoryWeapon)
    {
    ...
    void Inventory::addItem(DungeonKey dungeonKey)
    {
    ...
    void Inventory::addItem(DungeonItem dungeonItem)
    {
        switch (dungeonItem)
        {
    ...
    void Inventory::addItem(TradeItem tradeItem)
    {
    ...
    void Inventory::addItem(InventoryMiscItem inventoryMiscItem)
    {
    ...
    void Inventory::addItem(Instrument instrument)
    {
    ...
    void Inventory::addItem(Ruppee ruppees)
    {
    ...
    void Inventory::addItem(Tunic tunic)
    {
        assert(tunic > Tunic::TUNIC_NONE && tunic < Tunic::TUNIC_COUNT);
        m_tunic = tunic;
    }

    void Inventory::addItem(HeartPiece heartPiece)
    {
    ...
    void Inventory::addItem(int photograph)
    {
        m_photographs = std::min(m_photographs + photograph, MAX_PHOTOGRAPHS);
    }

    void Inventory::useItem(DungeonItem dungeonItem)
    {
        switch (dungeonItem)
        {
        case DungeonItem::ITEM_KEY:
    ...
    void Inventory::useItem(InventoryMiscItem inventoryMiscItem)
    {
        switch (inventoryMiscItem)
        {
        case ITEM_RED_POTION:
    ...
    void Inventory::removeItem(const InventoryWeapon& inventoryWeapon)
    {
    ...
    InventoryWeapon Inventory::weaponA() const
    {
        return m_weaponA;
    }

    InventoryWeapon Inventory::weaponB() const
    {
        return m_weaponB;
    }

    Vector<int> Inventory::dungeonMarkerLocation() const
    {
        return m_dungeonPosition;
    }

    void Inventory::setDungeonLocationMarker(const int x, const int y)
    {
    ...
    void Inventory::drawNumber(const Sprite& srcTexture, bool drawLevel, bool useNormalFont, int trailingDigits, int number, const Rect<int>& dstRect) const
    {
    ...
    void Inventory::drawWeaponLevel(const Sprite& srcTexture, WeaponItem weapon, const Rect<int>& dstRect)
    {
        switch (weapon)
        {
        case WPN_SHIELD:
    ...
    bool Inventory::itemExists(const InventoryWeapon& inventoryWeapon) const
    {
    ...
    bool Inventory::itemExists(WeaponItem item) const
    {
    ...
    bool Inventory::itemExists(DungeonKey dungeonKey) const
    {
        for (int i = 0; i < KEY_COUNT; i++)
        {
    ...
    WeaponLevel Inventory::itemLevel(WeaponItem item) const
    {
    ...
    void Inventory::inDungeon(bool inside)
    {
        m_inDungeon = inside;
    }
