...
struct InventoryItem
{
    ...
    {
        ostream << '(' << ENUM_VALUE(item.usuableItem) << ','
                << ENUM_VALUE(item.itemAttribute) << ',' << item.value << ')';
        return ostream;
    }

    bool operator==(const InventoryItem& item) const
    {
        return value == item.value && itemAttribute == item.itemAttribute
               && usuableItem == item.usuableItem;
    }
...
{
    return std::any_of(array.cbegin(),
                       array.cend(),
                       [&item](Enum exists) { return item == exists; });
}
}  // namespace Zelda