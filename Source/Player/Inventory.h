...
namespace Zelda
{
    ...
    public:
        ...
        bool magicPowder() const;
        bool arrows() const;
        bool bombs() const;

        void addItemQuantity(WeaponItem item, int quantity);
        void addItem(const InventoryWeapon& inventoryWeapon);
        void addItem(DungeonKey dungeonKey);
        void addItem(DungeonItem dungeonItem);
        void addItem(TradeItem tradeItem);
        void addItem(InventoryMiscItem inventoryMiscItem);
        void addItem(Instrument instrument);
        void addItem(Ruppee ruppees);
        void addItem(Tunic tunic);
        void addItem(HeartPiece heartPiece);
        void addItem(int photograph);

        void useItem(DungeonItem dungeonItem);
        void useItem(InventoryMiscItem inventoryMiscItem);

        void removeItem(const InventoryWeapon& inventoryWeapon);
        WeaponLevel itemLevel(WeaponItem item) const;

        void inDungeon(bool inside);
        ...
        InventoryWeapon weaponA() const;
        InventoryWeapon weaponB() const;

        // Set position of current location marker for dungeon
        Vector<int> dungeonMarkerLocation() const;
        void setDungeonLocationMarker(const int x, const int y);
        void getDungeonMap(Dungeon dungeon, DungeonMapEntry(&dungeonMapEntry)[DUNGEON_MAX_BLOCKS_X][DUNGEON_MAX_BLOCKS_Y]) const;
        void setDungeonMapEntry(const int x, const int y, const DungeonMapEntry& dungeonMapEntry);
    ...
    private:
        ...
        void drawNumber(const Sprite& srcTexture, bool drawLevel, bool useNormalFont, int trailingDigits, int number, const Rect<int>& dstRect) const;
        void drawWeaponLevel(const Sprite& srcTexture, WeaponItem weapon, const Rect<int>& dstRect);
        ...
        bool itemExists(const InventoryWeapon& inventoryWeapon) const;
        bool itemExists(WeaponItem item) const;
        bool itemExists(DungeonKey dungeonKey) const;
