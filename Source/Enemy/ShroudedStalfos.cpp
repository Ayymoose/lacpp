...
ShroudedStalfos::ShroudedStalfos(EnemyType enemyType, float x, float y) :
    Renderable("Shrouded Stalfos", ResourceManager::getInstance()[SpriteResource::SPR_ENEMY], ZD_DEPTH_ENEMY),
    Enemy(x, y),
    m_type(enemyType),
    m_steps(0)
{
...
void ShroudedStalfos::render() noexcept
{
    // TODO: Render wings if winged octorok and change behaviour when Link close
    auto animation = m_enemy[ENEMY_SHROUDED_STALFOS];
...
void ShroudedStalfos::update() noexcept
{
}

float ShroudedStalfos::health() const noexcept
{
    return m_health;
}

Vector<float> ShroudedStalfos::position() const noexcept
{
    return m_position;
}

void ShroudedStalfos::attack() noexcept
{
...
void ShroudedStalfos::die() noexcept
{
}
