...
void Dialogue::question(const std::string& question,
...
void Dialogue::render()
{
    ...
    Rect<int> dstRectChar = {TEXT_POS_X + m_dstCharX * m_dialogueImpl.charWidth(),
    ...
    {
        m_questionXPos = m_dialoguePosX
                         + (m_dialogueImpl.optionPaddingLength()
                            * (m_dialogueImpl.charWidth() - 1));
        m_questionYPos = m_dialogueImpl.dialogueY() + TEXT_POS_Y
                         + m_dialogueImpl.lineHeight();
    }
    ...
    if (m_currentChar < message.length())
    {
        ...
        if (!m_scrollMessage)
        {
            ...
            if ((m_currentChar && (m_currentChar % m_dialogueImpl.maxCharsPerLine() == 0))
                && !m_moreText)
            {
            ...
            else
            {
                if (!m_continue)
                {
                    m_dstCharX = std::min(m_dstCharX + 1,
                                          m_dialogueImpl.maxCharsPerLine() - 1);
                }
        ...
        else if (m_scrollMessage)
        {
            ...
            m_subTexture->colourSprite(Rect<int>{0,
                                                 0,
            ...
            const Rect<int> srcRectSubTextureLowerHalf = {0,
                                                          m_dstCharY,
                                                          m_dialogueImpl.dialogueWidth(),
        ...
        assert(dstRectChar.y
               < TEXT_POS_Y + m_dialogueImpl.maxLines() * m_dialogueImpl.lineHeight());
        assert(dstRectChar.x >= TEXT_POS_X);
        assert(dstRectChar.x < TEXT_POS_X
                                   + m_dialogueImpl.maxCharsPerLine()
                                         * m_dialogueImpl.charWidth());

        // Make text yellow
        Sprite::copySprite(*m_text,
                           *m_subTexture,
                           srcRectChar,
                           dstRectChar,
                           makeRGB(TEXT_R, TEXT_G, TEXT_B));
    }
    ...
    if (m_dialogueImpl.isQuestion()
        && (m_currentChar == m_dialogueImpl.message().length()) && m_toggleQuestion)
    {
        drawQuestionMarker();
    }
}
...
void Dialogue::control()
{
    if (m_continue && Keyboard::getInstance().keyPressed(BUTTON_B))
    {
        ...
        m_toggleArrow.reset();
             ...
             && Keyboard::getInstance().keyPressed(BUTTON_B))
    {
        reset();
    ...
    else if (m_dialogueImpl.isQuestion()
             && m_currentChar == m_dialogueImpl.message().length())
    {
        if (Keyboard::getInstance().keyPressed(BUTTON_RIGHT)
            && m_dialogueImpl.choice() != m_dialogueImpl.choice2())
        {
            m_questionXPos += (m_dialogueImpl.choice().length() + 2)
                              * m_dialogueImpl.charWidth();
            m_dialogueImpl.setChoice2();
            // If pressed right once
            // Choice is 2
        }
        else if (Keyboard::getInstance().keyPressed(BUTTON_LEFT)
...
Dialogue::Dialogue()
    : IRenderable("Dialogue",
                  Sprite(Renderer::getInstance().getRenderer(),
                         DialogueImpl::dialogueWidth(),
                         DialogueImpl::dialogueHeight()),
                  ZD_DEPTH_DIALOGUE)
    , Controllable(m_name)
    ...
    , m_subTexture(std::make_unique<Sprite>(Renderer::getInstance().getRenderer(),
                                            DialogueImpl::dialogueWidth(),
                                            DialogueImpl::dialogueHeight()))
    , m_moreText(false)
    , m_toggleQuestion(15)
    ...
    , m_questionYPos(0)
{
    ...
    m_sprite->colourSprite(Rect<int>{0, 0, m_sprite->width(), m_sprite->height()},
                           makeRGB(0, 0, 0));
    m_subTexture->colourSprite(Rect<int>{0, 0, m_sprite->width(), m_sprite->height()},
                               makeRGB(0, 0, 0));
}
...
void Dialogue::drawQuestionMarker() const
{
    ...
    m_questionMarker->drawSprite(Rect<int>{144,
                                           16,
                                           m_dialogueImpl.charWidth(),
                                           m_dialogueImpl.charHeight()},
                                 Rect<int>{m_questionXPos,
                                           m_questionYPos,
                                           m_dialogueImpl.charWidth(),
                                           m_dialogueImpl.charHeight()});
}
...
{
    m_redArrow->drawSprite(Rect<int>{136,
                                     16,
                                     m_dialogueImpl.charWidth(),
                                     m_dialogueImpl.charHeight()},
                           Rect<int>{m_dialoguePosX + ARROW_POS_X,
