...
Sprite::Sprite(SDL_Renderer* renderer, const int width, const int height)
{
    ...
    if (m_renderer)
    {
        assert(m_width);
        assert(m_height);
        m_sprite = SDL_CreateTexture(m_renderer,
...
Sprite::Sprite(SDL_Renderer* renderer, SDL_Surface* surface)
{
    ...
    auto const textureCreatedFromSurface =
        SDL_CreateTextureFromSurface(m_renderer, surface);
    SDL_FreeSurface(surface);

    // Query dimensions
    SDL_ASSERT(
        SDL_QueryTexture(textureCreatedFromSurface, nullptr, nullptr, &m_width, &m_height));
    assert(m_width);
    assert(m_height);
...
SDL_RendererFlip Sprite::flipToSDLRendererFlip(SpriteFlip flip)
{
    switch (flip)
    {
