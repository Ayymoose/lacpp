...
void DialogueImpl::message(const std::string& message, float yPos)
{
    ...
    m_messages.emplace(Message(message,
                               (yPos > DIALOGUE_HEIGHT + DIALOGUE_POS_Y_HIGH)
                                   ? DIALOGUE_POS_Y_HIGH
                                   : DIALOGUE_POS_Y_LOW));
}

void DialogueImpl::question(const std::string& question,
                            ...
                            float yPos)
{
    ...
    auto const messagePadding =
        MAX_CHAR_PER_LINE - (question.length() % MAX_CHAR_PER_LINE);
    ...
    assert(choice1.length() + OPTIONS_SPACE.length() + choice2.length() + 1
           < MAX_CHAR_PER_LINE);
    ...
    std::for_each(message.cbegin(),
                  ...
                  [](const char c)
                  {
                      ...
                      bool isSpecialCharacter =
                          (c == '#' || c == '<' || c == '>' || c == '@');
                      assert(isSpecialCharacter || isLowerCaseCharacter || isUppperCaseCharacter
                             || isDigit || isPunctuation || isSpace);
                  });
}
