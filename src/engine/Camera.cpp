...
Camera::Camera()
    : IRenderable("Camera",
                  Sprite(Renderer::getInstance().getRenderer(), CAMERA_WIDTH, CAMERA_HEIGHT),
                  ZD_DEPTH_BACKGROUND)
    , m_scrollX(0)
    , m_scrollY(0)
    , m_x(0)
...
void Camera::render()
{
    ...
    else if (y < m_scrollY - SCROLL_UP_EDGE && !m_scrollUp)
    {
        ...
        // m_nextRoomIndex /*- m_tilemap.roomsAcross()*/); RoomManager::getInstance().transitionObjects(m_nextRoomIndex/*
        // - m_tilemap.roomsAcross()*/, 0, CAMERA_HEIGHT);
    ...
    if (m_scrollLeft)
    {
        if (m_scrolled != CAMERA_WIDTH)
        {
            ...
            player->setPosition(player->position().x - pushSteps,
                                player->position().y);
        }
        else
        {
            ...
            player->setPosition(CAMERA_WIDTH + player->position().x,
                                player->position().y);
    ...
    else if (m_scrollRight)
    {
        ...
        else
        {
            ...
            player->setPosition(player->position().x - CAMERA_WIDTH,
                                player->position().y);
    ...
    else if (m_scrollUp)
    {
        ...
        else
        {
            ...
            player->setPosition(player->position().x,
                                CAMERA_HEIGHT + player->position().y);
    ...
    RoomManager::getInstance().updateCurrentRoomPosition(m_screenX - m_scrollX,
                                                         m_screenY - m_scrollY);
    RoomManager::getInstance().updateNextRoomPosition((m_screenX - m_scrollX) + m_swapX,
...
{
    assert((CAMERA_WIDTH % scrollSpeed == 0)
           && "scrollSpeed not multiple of CAMERA_WIDTH");
    assert((CAMERA_HEIGHT % scrollSpeed == 0)
           && "scrollSpeed not multiple of CAMERA_HEIGHT");
