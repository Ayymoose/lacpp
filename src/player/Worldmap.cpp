...
Worldmap::Worldmap()
    : IRenderable("Worldmap",
                  *ResourceManager::getInstance()[SpriteResource::SPR_WORLD_MAP],
                  ZD_DEPTH_WORLDMAP)
    , Controllable(m_name)
    , m_show(false)
    , m_scopeSelect(false)
...
void Worldmap::control()
{
    ...
    if (Keyboard::getInstance().keyPressed(BUTTON_B))
    {
        ...
        Dialogue::getInstance().message(m_worldMapImpl.locationName(),
                                        (WORLDMAP_START_Y + m_scopeY * 8) - 10);
    }
}
            ...
            if (!m_worldMapImpl.locationVisited(x, y))
            {
                ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
                    ->drawSprite(m_worldmapSrcSprites[WORLDMAP_AREA_UNVISITED],
                                 Rect<int>{WORLDMAP_START_X + x * 8,
                                           WORLDMAP_START_Y + y * 8,
                                           7,
                                           7});
            }
        }
...
void Worldmap::drawLocationImage() const
{
    ...
    if (m_worldMapImpl.locationType() != LT_NONE)
    {
        ...
        ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
            ->drawSprite(srcRectLocation, Rect<int>{sx, sy, 30, 30});
    }
}
...
void Worldmap::drawScope() const
{
    ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
        ->drawSprite(m_worldmapSrcSprites[WORLDMAP_AREA_SCOPE],
                     Rect<int>{(WORLDMAP_START_X + m_scopeX * 8) - 5,
    ...
    if (m_scopeSelect)
    {
        ...
        Rect<int> dstRect = {(WORLDMAP_START_X + m_scopeX * 8) - 5,
                             (WORLDMAP_START_Y + m_scopeY * 8) - 5,
                             8,
                             8};
        dstRect.w = 9;
        ...
        // Up arrow
        ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
            ->drawSpriteEx(srcRect, dstRect, 0, SpriteFlip::FLIP_NONE);

        // Right arrow
        dstRect.x += 15;
        dstRect.y += 14;
        ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
            ->drawSpriteEx(srcRect, dstRect, 90, SpriteFlip::FLIP_NONE);

        // Down arrow
        dstRect.x -= 15;
        dstRect.y += 15;
        ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
            ->drawSpriteEx(srcRect, dstRect, 180, SpriteFlip::FLIP_NONE);

        // Left arrow
        dstRect.x -= 15;
        dstRect.y -= 15;
        ResourceManager::getInstance()[SpriteResource::SPR_INVENTORY]
            ->drawSpriteEx(srcRect, dstRect, 270, SpriteFlip::FLIP_NONE);
    }
}
