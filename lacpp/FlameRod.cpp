...
void FlameRod::render(SDL_Renderer* pRenderer)
{
    ...
    // The flame
    DASSERT(SDL_RenderCopyEx(pRenderer, m_texture, &flameSrcRect, &flameDstRect, m_orientation, nullptr, SDL_FLIP_NONE) == 0, SDL_GetError());
    ...
    // The flame rod
    if (m_display)
    DASSERT(SDL_RenderCopyEx(pRenderer, m_texture, &flameRodSrcRect, &flameRodDstRect, m_flameRodOrientation, nullptr, SDL_FLIP_NONE) == 0, SDL_GetError());
